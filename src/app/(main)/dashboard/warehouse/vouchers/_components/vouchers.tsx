...
import { filters, type VoucherRow } from "./data";
import { vouchersColumns } from "./vouchers-columns";
import { VouchersTable } from "./vouchers-table";

export function Vouchers({ vouchers: initialVouchers }: { vouchers: VoucherRow[] }) {
  ...
  const warehouseFilter =
    (table.getColumn("warehouse")?.getFilterValue() as string | undefined) ?? filters.warehouse[0];
  ...
  return (
    <Card>
      <CardHeader className="border-b has-data-[slot=card-action]:grid-cols-1 md:has-data-[slot=card-action]:grid-cols-[1fr_auto]">
        <CardTitle className="text-xl leading-none">Vouchers</CardTitle>
        <CardDescription className="max-w-sm leading-snug">
          Review stock movement approvals, transfer records, and warehouse documents.
        </CardDescription>
      ...
      <CardContent className="flex flex-col gap-4 px-0">
        <div className="flex flex-wrap items-center justify-between gap-3 px-4">
          ...
          <Select value={warehouseFilter} onValueChange={(value) => setColumnSelectFilter("warehouse", value)}>
            <SelectTrigger size="sm">
